...
const Binaural: React.FC = (): JSX.Element => {
  ...
  const handlePresetClick = (presetFrequency1: number, presetFrequency2: number) => {
    if (isPlaying) {
      handlePause();
    }
    setFrequency1(presetFrequency1);
    setFrequency2(presetFrequency2);
    handlePlay();
  };
      ...
      <div className="flex flex-col gap-2">
        ...
        <div id="preset-buttons" className="flex justify-between items-center">
          <button
            ...
            onClick={() => handlePresetClick(200, 205)}
          >
            Focus
          </button>
          <button
            id="relaxButton"
            className="btn btn-accent"
            onClick={() => handlePresetClick(180, 174)}
          >
            Relaxation
          </button>
          <button
            id="meditationButton"
            className="btn btn-accent"
            onClick={() => handlePresetClick(136.1, 128.1)}
          >
            Meditation
          </button>
          <button
            id="creativityButton"
            className="btn btn-accent"
            onClick={() => handlePresetClick(240, 234)}
          >
            Creativity
          </button>
          <button
            id="energyButton"
            className="btn btn-accent"
            onClick={() => handlePresetClick(240, 244)}
          >
            Energy+
          </button>
          <button
            id="sleepButton"
            className="btn btn-accent"
            onClick={() => handlePresetClick(70, 66)}
          >
            Sleep
          </button>
        </div>
